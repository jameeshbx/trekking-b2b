    ...
    <div className="w-full bg-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-6 border-b border-gray-200 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="text-xs sm:text-sm font-medium text-green-900 font-semibold">Enquiry Details</div>
        </div>
        <div className="flex items-center gap-4">
          <div className="relative flex-1 sm:flex-initial">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#183F30] focus:border-transparent w-full sm:w-auto"
            />
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-6 gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">{customerName}</h1>
          <p className="text-sm text-gray-600">{phoneNumber}</p>
        </div>
        <button
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors self-start sm:self-auto"
          onClick={goBack}
        >
      ...
      {/* Progress Steps */}
      <div className="px-4 sm:px-7 pb-8 sm:pb-16 pt-3">
              ...
              return (
                <div key={step.id} className="flex items-center gap-4">
                  ...
                  {index < steps.length - 1 && (
                    <div className="absolute left-8 mt-8 w-0.5 h-6 bg-gray-200 z-0"></div>
                  )}
                  ...
                  <div className="flex-1">
                    <div
                      className={`
                        ...
                        ${isCompleted ? "bg-[#14A155] text-white" : "bg-gray-100 text-gray-600"}
                        ${isCurrent ? "bg-[#14A155] text-white" : ""}
                      `}
                    >
                      {step.label}
                    </div>
...
const EnquiryStatusDemo = () => {
  ...
  useEffect(() => {
    const loadEnquiryData = async () => {
      ...
      // Try to get data from URL params (if available)
      if (typeof window !== 'undefined') {
